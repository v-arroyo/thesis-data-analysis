import pandas as pd
import plotly.express as px
from sqlalchemy import create_engine
import plotly.io as pio
import plotly.graph_objects as go
import os
from dotenv import load_dotenv

load_dotenv()

engine = create_engine(f'mysql+pymysql://{os.getenv("DB_USER")}:{os.getenv("DB_PASSWORD")}@localhost/{os.getenv("DB_NAME")}')

query = """
SELECT
	b.temp_early, 
	b.temp_late,
    b.temp,
	b.social_group,
	form,
    form2,
	COUNT(amulet_id) AS total
FROM amulets a
JOIN burials b ON b.burial_id = a.burial_id
WHERE dating = 'napatan' 
	AND b.site_id IN (4,5,6,7,8,9,10)
	AND a.form NOT IN ('hathor')
    AND a.form2 NOT IN ('jackal', 'scorpion', 'lunar crescent')
    AND a.type = 'deity'
GROUP BY 1,2,3,4,5,6
"""

df = pd.read_sql(query, engine)

custom_colors = ['#C0C0C0']

phase_order = ["pre-25th", "25th", "EN", "MN", "LN"]

# expanded_rows = []

# aggregate animals by phase, social group, and forms
df_grouped = df.groupby(['temp', 'social_group', 'form', 'form2'], as_index=False)['total'].sum()

df_grouped = df_grouped.sort_values(['temp', 'form'])

fig = px.scatter(
    df_grouped,
    x='form',
    y='form2',
    color='temp',
    text='total',
    facet_col='temp',
    facet_row='social_group',
    template="plotly_white",
    #title='Distribution of amulets combining two deities by social group and chronological phase',
    color_discrete_sequence=custom_colors,
    category_orders={"phase": phase_order, "social_group": ["non-elite"]}
)

fig.update_layout(xaxis=dict(automargin=True, title_standoff=0),
        legend=dict(
        orientation="h",
        yanchor="bottom",
        y=-0.30,
        xanchor="center",
        x=0.40),
        #traceorder='reversed'),
    font=dict(
        family="Verdana, sans-serif",
        color='black',
        size=8),
    legend_title_text='',
    title_font=dict(size=8),
    autosize=True,
    margin=dict(l=0, r=0, t=20, b=0),
    showlegend=False
)

for annotation in fig.layout.annotations:
    if annotation.text.startswith("social_group="):
        annotation.text = annotation.text.replace("social_group=", "")

for annotation in fig.layout.annotations:
    if annotation.text.startswith("temp="):
        annotation.text = annotation.text.replace("temp=", "")

fig.update_traces(textposition='middle right', textfont_size=6)
fig.update_yaxes(title='', nticks=20)
fig.update_xaxes(title='', matches=None)

pio.write_image(fig, 'images/chapter6/motifs_phase_2x-non-elite.png',scale=3, width=550, height=220)